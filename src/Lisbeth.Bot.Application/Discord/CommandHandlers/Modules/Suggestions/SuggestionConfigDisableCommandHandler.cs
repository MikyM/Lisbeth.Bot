...
[UsedImplicitly]
public class SuggestionConfigDisableCommandHandler : IAsyncCommandHandler<SuggestionConfigDisableCommand,DiscordEmbed>
{
    ...
    public async Task<Result<DiscordEmbed>> HandleAsync(SuggestionConfigDisableCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!guild.IsSuggestionModuleEnabled)
            return new ArgumentInvalidError(nameof(guild.SuggestionConfig),"Suggestion module is already disabled, please use the enable action.");
