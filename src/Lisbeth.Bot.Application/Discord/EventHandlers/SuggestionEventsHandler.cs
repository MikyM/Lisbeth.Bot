...
[UsedImplicitly]
public class SuggestionEventsHandler : IDiscordMessageEventsSubscriber
{
    private readonly ICommandHandler<HandlePossibleSuggestionCommand> _commandHandler;

    public SuggestionEventsHandler(IDiscordService discord, ICommandHandler<HandlePossibleSuggestionCommand> commandHandler)
    {
    ...
    public async Task DiscordOnMessageCreated(DiscordClient sender, MessageCreateEventArgs args)
    {
        ...
        await _commandHandler.HandleAsync(new HandlePossibleSuggestionCommand(args));
    }
