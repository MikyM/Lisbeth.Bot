...
{
    public class TicketRemoveReqValidator : AbstractValidator<TicketRemoveReqDto>
    {
        public TicketRemoveReqValidator(IDiscordService discordService) : this(discordService.Client)
        {
        }

        public TicketRemoveReqValidator(DiscordClient discord)
        {
            ...
            RuleFor(x => x.GuildId)
                ...
                .DependentRules(x => x.SetAsyncValidator(new DiscordGuildIdValidator<TicketRemoveReqDto>(discord)));
            RuleFor(x => x.ChannelId)
                .NotEmpty()
                .When(x => !x.Id.HasValue && !x.GuildId.HasValue && x.GuildSpecificId.HasValue && x.OwnerId.HasValue)
                .DependentRules(x => x.SetAsyncValidator(new DiscordChannelIdValidator<TicketRemoveReqDto>(discord)));
            RuleFor(x => x.OwnerId)
                .NotEmpty()
            ...
            RuleFor(x => x.RequestedOnBehalfOfId)
                .NotEmpty()
                .DependentRules(x => x.SetAsyncValidator(new DiscordUserIdValidator<TicketRemoveReqDto>(discord)));

            RuleFor(x => x.SnowflakeId)
                .NotEmpty()
                .DependentRules(x => x.SetAsyncValidator(new DiscordSnowflakeIdValidator<TicketRemoveReqDto>(discord)));
        }
    }
}